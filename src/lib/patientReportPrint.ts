...
export function getPatientReportHtml(data: PatientReportData): string {
  ...
  const weightVal = latestVital?.weight || '';
  const rrVal = latestVital?.rr || latestVital?.respiration || '';
  const cbsVal = latestVital?.cvs || latestVital?.cbs || '';
  const rsVal = latestVital?.rs || '';
  const cnsVal = latestVital?.cns || '';
  const paVal = latestVital?.perAbdomen || latestVital?.per_abdomen || latestVital?.pa || '';
  const localExamVal = latestVital?.localExam || latestVital?.local_exam || '';
  const inputOutputVal = latestVital?.inputOutput || latestVital?.input_output || '';
        ...
        <div class="report-container">
          ...
          <div class="meta-box">
            <div class="grid-2">
              ...
              <div style="border-left: 1px solid #cbd5e1; padding-left: 15px;">
                ...
                ${patient.fatherName ? `<div><strong>Guardian / Relative:</strong> ${patient.fatherName} ${patient.fatherPhone ? `(Ph: ${patient.fatherPhone})` : ''}</div>` : ''}
          ...
          <div class="vitals-box">
            ...
            <div class="vitals-grid">
              ...
              ${weightVal ? `<div class="vital-item"><div class="vital-label">Weight</div><div class="vital-value">${weightVal} kg</div></div>` : ''}
              ${rrVal ? `<div class="vital-item"><div class="vital-label">Resp Rate</div><div class="vital-value">${rrVal} /min</div></div>` : ''}
              ${cbsVal ? `<div class="vital-item"><div class="vital-label">CVS</div><div class="vital-value">${cbsVal}</div></div>` : ''}
              ${rsVal ? `<div class="vital-item"><div class="vital-label">RS</div><div class="vital-value">${rsVal}</div></div>` : ''}
              ${cnsVal ? `<div class="vital-item"><div class="vital-label">CNS</div><div class="vital-value">${cnsVal}</div></div>` : ''}
              ${paVal ? `<div class="vital-item"><div class="vital-label">P/A (Abdomen)</div><div class="vital-value">${paVal}</div></div>` : ''}
              ${localExamVal ? `<div class="vital-item"><div class="vital-label">Local Exam</div><div class="vital-value">${localExamVal}</div></div>` : ''}
              ${inputOutputVal ? `<div class="vital-item"><div class="vital-label">Input / Output</div><div class="vital-value">${inputOutputVal}</div></div>` : ''}
            </div>
          </div>
          ...
          <div style="margin-top: 40px; display: flex; justify-content: space-between; align-items: flex-end; border-top: 1px solid #cbd5e1; padding-top: 15px; page-break-inside: avoid;">
            <div style="font-size: 9px; color: #94a3b8; font-weight: 500;">
              * Digitally authenticated Electronic Health Record dossier printed on behalf of medical staff.<br>
              Any clinical revisions must be completed via Hospital Clinical Portal. 
            </div>
            <div style="text-align: right;">
              <div style="font-size: 11px; font-weight: bold; color: #1e293b;">Authenticated By Signature:</div>
              <div style="font-family: cursive; font-size: 16px; color: #4338ca; border-bottom: 1.5px solid #e2e8f0; padding-bottom: 2px; margin-top: 4px; display: inline-block;">Hospital Medical Record Desk</div>
            </div>
          </div>
        </div>
      </body>
