...
export default function InventoryPurchase() {
  ...
  const handleExportInventoryPurchase = () => {
    try {
      let csvContent = '';
      let filename = `Inventory_${activeTab}_Export_${new Date().toISOString().split('T')[0]}.csv`;

      if (activeTab === 'pos') {
        const headers = ['PO Number', 'Vendor / Supplier', 'Item', 'Order Quantity', 'Cost (₹)', 'Date', 'Status'];
        const rows = pos.map(p => [p.poNumber, p.vendor, p.item, p.orderQty, p.cost, p.date, p.status]);
        csvContent = [headers, ...rows].map(r => r.map(c => `"${String(c || '').replace(/"/g, '""')}"`).join(',')).join('\n');
      } else if (activeTab === 'grns') {
        const headers = ['GRN Number', 'PO Ref', 'Item Received', 'Supplier', 'Quantity', 'Batch No', 'Expiry Date', 'Receipt Date'];
        const rows = grns.map(g => [g.grnNumber, g.poNumber || 'N/A', g.item, g.supplier, g.receivedQty, g.batchNo, g.expiry || 'N/A', g.date]);
        csvContent = [headers, ...rows].map(r => r.map(c => `"${String(c || '').replace(/"/g, '""')}"`).join(',')).join('\n');
      } else if (activeTab === 'transfers') {
        const headers = ['Transfer ID', 'Item', 'Quantity', 'From Location', 'To Location', 'Date', 'Status'];
        const rows = transfers.map(t => [t.id, t.item, t.qty, t.fromLocation, t.toLocation, t.date, t.status]);
        csvContent = [headers, ...rows].map(r => r.map(c => `"${String(c || '').replace(/"/g, '""')}"`).join(',')).join('\n');
      } else if (activeTab === 'consumption') {
        const headers = ['Consumption ID', 'Item', 'Quantity', 'Patient MRN', 'Ward / Dept', 'Date', 'Staff Recorded'];
        const rows = consumptions.map(c => [c.id, c.item, c.qty, c.mrn || 'N/A', c.ward, c.date, c.staff]);
        csvContent = [headers, ...rows].map(r => r.map(c => `"${String(c || '').replace(/"/g, '""')}"`).join(',')).join('\n');
      } else if (activeTab === 'vendors') {
        const headers = ['Vendor ID', 'Company Name', 'Contact Phone', 'Email', 'Status', 'Compliance Rating', 'Address'];
        const rows = vendors.map(v => [v.id, v.name, v.contact, v.email, v.status, v.rating, v.address]);
        csvContent = [headers, ...rows].map(r => r.map(c => `"${String(c || '').replace(/"/g, '""')}"`).join(',')).join('\n');
      } else {
        const headers = ['Item Code', 'Item Name', 'Category', 'Stock on Hand', 'Unit', 'Safety Reorder Level', 'Unit Cost (₹)', 'Location'];
        const rows = items.map(i => [i.code, i.name, i.category, i.stock, i.unit, i.minLevel, i.unitCost, i.location]);
        csvContent = [headers, ...rows].map(r => r.map(c => `"${String(c || '').replace(/"/g, '""')}"`).join(',')).join('\n');
      }
      ...
      toast.success(`Inventory ${activeTab.toUpperCase()} exported to CSV successfully!`);
    } catch {
    ...
    <div id="inventory-purchase-management" className="p-4 lg:p-8 space-y-6">
      ...
      <div className="bg-[#1A5E63] rounded-3xl p-6 text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-md">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <Button onClick={handleExportInventoryPurchase} variant="outline" className="bg-white/10 hover:bg-white/20 text-white border-white/30 font-bold text-xs h-9 rounded-full px-3.5 cursor-pointer shadow-sm">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
        ...
        <div className="bg-white p-4 rounded-xl border shadow-sm col-span-2 flex flex-col justify-between">
          ...
          <p className="text-lg font-black text-emerald-700 mt-1">{formatCurrency(items.reduce((acc, curr) => acc + (curr.stock * curr.unitCost), 0))}</p>
        </div>
      </div>
                  ...
                  {filteredItems.map(item => {
                    ...
                    return (
                      <tr key={item.code} className="hover:bg-slate-50/50">
                        ...
                        <td className="p-3 font-medium text-slate-600">{formatCurrency(item.unitCost)}</td>
                        <td className="p-3 font-bold text-slate-700">{formatCurrency(item.stock * item.unitCost)}</td>
                ...
                <tbody className="divide-y text-xs">
                  {pos.map(po => (
                    <tr key={po.poNumber} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-3 font-black text-slate-700">{formatCurrency(Number(po.cost))}</td>
            ...
            <form onSubmit={handleCreatePO} className="p-6 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <SelectContent>
                    {items.map(itm => (
                      <SelectItem key={itm.code} value={itm.name}>{itm.name} ({formatCurrency(itm.unitCost)}/unit)</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
