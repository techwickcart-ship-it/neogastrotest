...
export default function BloodBank() {
  ...
  const handleExportBloodBank = () => {
    try {
      let csvContent = '';
      if (activeTab === 'donors') {
        const headers = ['Donor ID', 'Name', 'Age', 'Gender', 'Blood Group', 'Blood Pressure', 'Hemoglobin (g/dL)', 'Status', 'Last Donation'];
        const rows = donors.map(d => [d.id, d.name, d.age, d.gender, d.bloodGroup, d.bp, d.hemoglobin, d.status, d.lastDonation || 'N/A']);
        csvContent = [headers, ...rows].map(r => r.map(c => `"${String(c || '').replace(/"/g, '""')}"`).join(',')).join('\n');
      } else if (activeTab === 'stock' || activeTab === 'issues') {
        const headers = ['Issue ID', 'Recipient Name', 'MRN', 'Blood Group', 'Bag Number', 'Ward / OT', 'Date', 'Cross-Match', 'Issued By'];
        const rows = issues.map(i => [i.id, i.recipientName, i.mrn, i.bloodGroup, i.bagNo, i.ward, i.date, i.crossMatched || 'Compatible', i.issuedBy]);
        csvContent = [headers, ...rows].map(r => r.map(c => `"${String(c || '').replace(/"/g, '""')}"`).join(',')).join('\n');
      } else {
        const headers = ['Blood Group', 'Available Bags (Units)', 'Safe Storage Range', 'Status'];
        const rows = Object.entries(stocks).map(([bg, qty]) => [bg, qty, '2°C - 6°C', Number(qty) <= 4 ? 'Low Reserve' : 'Adequate']);
        csvContent = [headers, ...rows].map(r => r.map(c => `"${String(c || '').replace(/"/g, '""')}"`).join(',')).join('\n');
      }
      ...
      a.href = url;
      a.download = `Blood_Bank_${activeTab}_Register_${new Date().toISOString().split('T')[0]}.csv`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
      toast.success(`Blood Bank ${activeTab} data exported to CSV successfully!`);
    } catch {
      toast.error('Failed to export Blood Bank data');
    }
  };
    ...
    <div id="blood-bank" className="p-4 lg:p-8 space-y-6">
      ...
      <div className="bg-[#1A5E63] rounded-3xl p-6 text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-md">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button onClick={handleExportBloodBank} variant="outline" className="bg-white/10 hover:bg-white/20 text-white border-white/30 font-bold gap-2 rounded-full px-4 py-5 h-auto text-xs shadow-sm cursor-pointer">
