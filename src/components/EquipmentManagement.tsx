...
const SAMPLE_EQUIPMENT = [
  { id: 'EQ001', name: 'GE Optima CT Scanner', department: 'Radiology', model: 'GE Optima 660', serialNumber: 'CT-992384-A', installDate: '2023-04-12', status: 'Operational', lastPmDate: '2026-05-10', nextPmDate: '2026-11-10', amcVendor: 'GE Healthcare Services', amcExpiry: '2027-04-12', location: 'CT Scan Room 1' },
  { id: 'EQ002', name: 'Mindray BeneHeart Defibrillator', department: 'Emergency', model: 'Mindray D3', serialNumber: 'DF-112349-B', installDate: '2024-01-15', status: 'Operational', lastPmDate: '2026-06-01', nextPmDate: '2026-12-01', amcVendor: 'Mindray Diagnostics Ltd', amcExpiry: '2025-12-31', location: 'ER Trauma Room A' },
  { id: 'EQ003', name: 'Maquet FLOW-i Anesthesia Machine', department: 'OT / Surgery', model: 'Maquet C20', serialNumber: 'AN-775612-F', installDate: '2022-09-20', status: 'Under Maintenance', lastPmDate: '2025-09-15', nextPmDate: '2026-03-15', amcVendor: 'Getinge Medical India', amcExpiry: '2026-09-19', location: 'OT Room 2' },
  { id: 'EQ004', name: 'Dräger Babylog Ventilator', department: 'ICU / Neonatal', model: 'Dräger VN500', serialNumber: 'VT-441299-X', installDate: '2023-11-05', status: 'Calibration Due', lastPmDate: '2025-11-05', nextPmDate: '2026-05-05', amcVendor: 'Draeger Medical Solutions', amcExpiry: '2026-11-04', location: 'Neonatal ICU Bed 4' },
  { id: 'EQ005', name: 'Olympus CV-190 Endoscopy Processor', department: 'Gastroenterology', model: 'Evis Exera III', serialNumber: 'EN-884910-Y', installDate: '2024-06-10', status: 'Broken Down', lastPmDate: '2025-12-10', nextPmDate: '2026-06-10', amcVendor: 'Olympus Medical India', amcExpiry: '2026-06-09', location: 'Endoscopy Suite A' }
];
...
const normalizeEquipment = (eq: any) => {
  ...
  return {
    id: eq.id || '',
    name: eq.name || '',
    department: eq.department || 'Radiology',
    model: eq.model || '',
    serialNumber: eq.serialNumber || eq.serial_number || '',
    serial_number: eq.serialNumber || eq.serial_number || '',
    installDate: eq.installDate || eq.install_date || '',
    install_date: eq.installDate || eq.install_date || '',
    status: eq.status || 'Operational',
    lastPmDate: eq.lastPmDate || eq.last_pm_date || '',
    last_pm_date: eq.lastPmDate || eq.last_pm_date || '',
    nextPmDate: eq.nextPmDate || eq.next_pm_date || '',
    next_pm_date: eq.nextPmDate || eq.next_pm_date || '',
    amcVendor: eq.amcVendor || eq.amc_vendor || '',
    amc_vendor: eq.amcVendor || eq.amc_vendor || '',
    amcExpiry: eq.amcExpiry || eq.amc_expiry || '',
    amc_expiry: eq.amcExpiry || eq.amc_expiry || '',
...
export default function EquipmentManagement() {
  ...
  const [equipForm, setEquipForm] = useState({
    id: '', name: '', department: 'Radiology', model: '', serialNumber: '', installDate: '', status: 'Operational', lastPmDate: '', nextPmDate: '', amcVendor: '', amcExpiry: '', location: ''
  });
  ...
  const fetchData = async () => {
    try {
      ...
      if (eqData && eqData.length === 0) {
        ...
        setEquipment(((reEqData || SAMPLE_EQUIPMENT) as any[]).map(normalizeEquipment));
      } else {
        setEquipment(((eqData || SAMPLE_EQUIPMENT) as any[]).map(normalizeEquipment));
      }
  ...
  const handleAddEquipment = async (e: FormEvent) => {
    ...
    setEquipForm({ id: '', name: '', department: 'Radiology', model: '', serialNumber: '', installDate: '', status: 'Operational', lastPmDate: '', nextPmDate: '', amcVendor: '', amcExpiry: '', location: '' });
  };
  ...
  const filteredEq = equipment.filter(eq => {
    ...
    return name.includes(q) || model.includes(q) || serial.includes(q) || dept.includes(q) || loc.includes(q);
  });
                    ...
                    filteredEq.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="p-3">
                          ...
                          <p className="text-[10px] text-teal-600 font-semibold">{item.location || 'Unassigned'}</p>
                        </td>
                        <td className="p-3">
                        ...
                        <td className="p-3 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            <Button 
                              ...
                              onClick={() => {
                                setEquipForm(normalizeEquipment(item));
                                setIsEquipModalOpen(true);
                              }}
                            >
