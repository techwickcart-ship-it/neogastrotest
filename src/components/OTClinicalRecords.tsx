...
export default function OTClinicalRecords({ record, patient, surgeonName, onClose, onSaveNotes }: ClinicalWorkflowProps) {
  ...
  const handleSavePreOp = (e: React.FormEvent) => {
    ...
    toast.success('Pre-operative clinical assessment locked & finalized successfully!');
  };
  ...
  const handleAddConsumable = () => {
    ...
    const updatedInventory = otInventory.map(item => {
      if (item.id === invItem.id) {
        const newStock = Math.max(0, item.stock - useQty);
        if (newStock <= (item.minStockLevel || 3)) {
          toast.warning(`OT Stock Alert: ${item.name} stock is now low (${newStock} ${item.unit}s remaining)! Auto-maintenance alert logged.`, { duration: 5000 });
        }
    ...
    // Auto-maintain Inventory Transactions Log
    try {
      const newTx = {
        id: `tx-ot-${Date.now()}`,
        type: 'CONSUMPTION',
        source: 'OT Operation Theatre',
        itemCode: invItem.code || 'OT-ITEM',
        itemName: invItem.name,
        quantity: useQty,
        unit: invItem.unit || 'Pcs',
        date: new Date().toISOString().split('T')[0],
        patientId: patient?.id || record?.patientId || '',
        notes: `Auto-deducted during OT Surgery procedure: ${invItem.name} (Qty: ${useQty})`
      };
      const existingTxs = storage.get('hms_inventory_transactions', []);
      storage.set('hms_inventory_transactions', [newTx, ...existingTxs].slice(0, 500));
    } catch (txErr) {
      console.warn('Failed to record OT inventory transaction log:', txErr);
    }
        ...
        {activeSubTab === 'preop' && (
          <form onSubmit={handleSavePreOp} className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-extrabold text-slate-800 text-lg flex items-center gap-2">
                  ...
                  {clinicalData.preOp?.isLocked && (
                    <Badge className="bg-emerald-600 text-white font-extrabold text-[10px] gap-1 px-2.5 py-0.5">
              ...
              <Badge className={clinicalData.preOp?.isLocked ? "bg-emerald-100 text-emerald-800 border-emerald-300 uppercase text-[9px] font-extrabold tracking-widest h-6 px-3" : "bg-amber-100 text-amber-800 border-amber-300 uppercase text-[9px] font-extrabold tracking-widest h-6 px-3"}>
                {clinicalData.preOp?.isLocked ? "Locked File" : "Draft Record"}
              </Badge>
            </div>

            <fieldset disabled={!!clinicalData.preOp?.isLocked} className="space-y-4 disabled:opacity-85">
            ...
            <div className="pt-2 flex items-center gap-3">
              {clinicalData.preOp?.isLocked ? (
                <>
                  <Button type="button" disabled className="bg-emerald-700 text-white text-xs font-bold h-9 gap-2 cursor-not-allowed">
                    <Lock className="w-3.5 h-3.5" /> Pre-Op File Locked & Finalized
                  </Button>
                  <Button 
                    type="button" 
                    variant="outline" 
                    size="sm"
                    onClick={() => {
                      const updated = { ...clinicalData, preOp: { ...clinicalData.preOp, isLocked: false } };
                      saveToStorage(updated);
                      toast.info('Pre-Op File unlocked for editing');
                    }}
                    className="text-xs text-amber-700 border-amber-300 hover:bg-amber-50 h-9 font-bold"
                  >
                    Unlock File
                  </Button>
                </>
              ) : (
                <Button type="submit" className="bg-[#1A5E63] hover:bg-[#14484c] text-white text-xs font-bold h-9 gap-2">
                  <Lock className="w-3.5 h-3.5" /> Lock Pre-Op File
                </Button>
              )}
            </div>
          </form>
        ...
        {activeSubTab === 'anesthesia' && (
          <form onSubmit={handleSaveAnesthesia} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Interop Anesthesia Notes */}
              <div className="col-span-2 space-y-1.5 p-3 bg-teal-50/40 border border-teal-200/80 rounded-xl">
                <div className="flex items-center justify-between">
                  <Label className="font-extrabold text-teal-950 text-xs">Interop / Intra-Op Anesthesia Notes</Label>
                  <Badge variant="outline" className="bg-white text-[10px] font-bold text-teal-800">Interop Events</Badge>
                </div>
                <textarea 
                  value={clinicalData.anesthesia.interopNotes || ''} 
                  onChange={(e) => saveToStorage({ ...clinicalData, anesthesia: { ...clinicalData.anesthesia, interopNotes: e.target.value } })}
                  rows={2} 
                  className="w-full text-xs p-2.5 rounded-lg border border-teal-200 bg-white focus:outline-none focus:ring-2 focus:ring-teal-600 font-medium"
                  placeholder="Document intra-operative anesthesia events, airway stability, IV fluid boluses, muscle relaxant top-ups..."
                />
                <div className="flex flex-wrap gap-1.5 pt-1">
                  <span className="text-[10px] font-bold text-slate-500 self-center">Quick Interop Notes:</span>
                  <button 
                    type="button" 
                    onClick={() => saveToStorage({ ...clinicalData, anesthesia: { ...clinicalData.anesthesia, interopNotes: ((clinicalData.anesthesia.interopNotes || '') + ' Hemodynamically stable interop.').trim() } })}
                    className="text-[10px] bg-white hover:bg-teal-100 text-teal-900 font-bold px-2 py-0.5 rounded border border-teal-200 transition-colors"
                  >
                    + Stable Interop
                  </button>
                  <button 
                    type="button" 
                    onClick={() => saveToStorage({ ...clinicalData, anesthesia: { ...clinicalData.anesthesia, interopNotes: ((clinicalData.anesthesia.interopNotes || '') + ' ETT position verified at 21cm mark.').trim() } })}
                    className="text-[10px] bg-white hover:bg-teal-100 text-teal-900 font-bold px-2 py-0.5 rounded border border-teal-200 transition-colors"
                  >
                    + ETT Verified
                  </button>
                  <button 
                    type="button" 
                    onClick={() => saveToStorage({ ...clinicalData, anesthesia: { ...clinicalData.anesthesia, interopNotes: ((clinicalData.anesthesia.interopNotes || '') + ' Interop analgesia supplemented with Fentanyl 50mcg.').trim() } })}
                    className="text-[10px] bg-white hover:bg-teal-100 text-teal-900 font-bold px-2 py-0.5 rounded border border-teal-200 transition-colors"
                  >
                    + Analgesia Bolus
                  </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1.5">
                  ...
                  <Select 
                    value={String(clinicalData.postOp.aldreteScore ?? 10)} 
              ...
              <div className="space-y-1.5 col-span-2">
                ...
                <Select 
                  value={clinicalData.postOp.destination || 'IPD Ward (Bed B2)'} 
