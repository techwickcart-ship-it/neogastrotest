...
export default function LISAdvancedModules({ readOnly: _readOnly }: { readOnly?: boolean }) {
              ...
              {franchises.length === 0 ? (
                ...
                franchises.map(fran => (
                  <TableRow key={fran.centerId} className="hover:bg-slate-50/30 text-xs border-slate-100">
                    ...
                    <TableCell className={`py-2.5 text-center font-extrabold ${fran.outstandingBalance > 30000 ? 'text-red-600' : 'text-slate-800'}`}>
