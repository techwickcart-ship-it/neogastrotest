...
export default function LISTestMasters({ readOnly }: { readOnly?: boolean }) {
  ...
  const [categories, setCategories] = useState<TestCategory[]>(() => {
    const saved = storage.get('lis_categories', null);
    if (!saved || !Array.isArray(saved) || saved.length < MOCK_CATEGORIES.length) {
      storage.set('lis_categories', MOCK_CATEGORIES);
      return MOCK_CATEGORIES;
    }
    return saved;
  });

  const [subCategories, setSubCategories] = useState<TestSubCategory[]>(() => {
    const saved = storage.get('lis_subcategories', null);
    if (!saved || !Array.isArray(saved) || saved.length < MOCK_SUBCATEGORIES.length) {
      storage.set('lis_subcategories', MOCK_SUBCATEGORIES);
      return MOCK_SUBCATEGORIES;
    }
    return saved;
  });

  const [investigations, setInvestigations] = useState<InvestigationTest[]>(() => {
    const saved = storage.get('lis_investigations', null);
    if (!saved || !Array.isArray(saved) || saved.length < MOCK_INVESTIGATIONS.length) {
      storage.set('lis_investigations', MOCK_INVESTIGATIONS);
  ...
  const [parameters, setParameters] = useState<Parameter[]>(() => {
    const saved = storage.get('lis_parameters', null);
    if (!saved || !Array.isArray(saved) || saved.length < MOCK_PARAMETERS.length) {
      storage.set('lis_parameters', MOCK_PARAMETERS);
  ...
  // Re-sync all master tests (Pathology + Radiology)
  const handleReloadMasterList = () => {
    storage.set('lis_categories', MOCK_CATEGORIES);
    storage.set('lis_subcategories', MOCK_SUBCATEGORIES);
    storage.set('lis_investigations', MOCK_INVESTIGATIONS);
    storage.set('lis_parameters', MOCK_PARAMETERS);
    ...
    toast.success(`Successfully synchronized all ${MOCK_INVESTIGATIONS.length} Master Tests (Pathology + Radiology) with complete parameters & modalities!`);
  };
  ...
  // Investigation Pagination States
  const [investigationPage, setInvestigationPage] = useState(1);
  const [investigationPageSize, setInvestigationPageSize] = useState(25);

  const filteredInvestigations = useMemo(() => {
    const q = (testSearch || '').toLowerCase().trim();
    if (!q) return investigations;
    return investigations.filter(t => 
  ...
  const totalInvestigationPages = useMemo(() => {
    if (investigationPageSize === -1 || filteredInvestigations.length === 0) return 1;
    return Math.ceil(filteredInvestigations.length / investigationPageSize);
  }, [filteredInvestigations.length, investigationPageSize]);

  const paginatedInvestigations = useMemo(() => {
    if (investigationPageSize === -1) return filteredInvestigations;
    const start = (investigationPage - 1) * investigationPageSize;
    return filteredInvestigations.slice(start, start + investigationPageSize);
  }, [filteredInvestigations, investigationPage, investigationPageSize]);
  ...
  // Editing state for Investigation
  const [editingTestCode, setEditingTestCode] = useState<string | null>(null);

  const handleStartEditTest = (test: InvestigationTest) => {
    setEditingTestCode(test.code);
    setNewTest({
      code: test.code,
      name: test.name,
      shortName: test.shortName || test.name,
      department: test.department || 'Pathology',
      categoryId: test.categoryId || '',
      subCategoryId: test.subCategoryId || '',
      sampleType: test.sampleType || 'SST Serum',
      method: test.method || 'Automated Analyzer',
      machineName: test.machineName || 'General Analyzer',
      reportType: test.reportType || 'Quantitative',
      tat: test.tat || '4 Hours',
      normalRangeApplicable: !!test.normalRangeApplicable,
      criticalValueApplicable: !!test.criticalValueApplicable,
      nablCompliance: !!test.nablCompliance,
      activeStatus: test.activeStatus || 'Active',
      price: test.price || 200
    });
  ...
  const handleDeleteTest = (code: string) => {
    if (!checkPermission()) return;
    const updated = investigations.filter(t => t.code !== code);
    setInvestigations(updated);
    storage.set('lis_investigations', updated);
    toast.info('Test removed from investigation directory');
  };

  const handleAddTest = () => {
    if (!checkPermission()) return;
    if (!newTest.code || !newTest.name) {
      toast.error('Please fill in Test Code and Test Name');
      return;
    }
    const testToAdd: InvestigationTest = {
      ...
      categoryId: newTest.categoryId || 'CAT-GEN',
      subCategoryId: newTest.subCategoryId || 'SUB-GEN',
    ...
    let updated: InvestigationTest[];
    if (editingTestCode) {
      updated = investigations.map(t => t.code === editingTestCode ? testToAdd : t);
      toast.success('Investigation Test details updated!');
    } else {
      updated = [...investigations, testToAdd];
          ...
          <Card className="border-none shadow-sm shadow-slate-100 bg-white rounded-2xl overflow-hidden">
            <CardHeader className="pb-4 border-b border-slate-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                ...
                <Button 
                  variant="outline"
                  onClick={handleReloadMasterList}
                  className="border-emerald-200 bg-emerald-50/50 hover:bg-emerald-100/70 text-emerald-800 font-semibold text-xs h-9 rounded-lg gap-1.5 shrink-0"
                  title="Reload all Pathology (105) & Radiology (92) Master Tests"
                >
                  <RefreshCw className="w-3.5 h-3.5 text-emerald-600" /> Sync Master Tests ({MOCK_INVESTIGATIONS.length})
                </Button>
                <RateListExcelUploader 
                  variant="compact"
                  onImportSuccess={() => {
                    const freshLis = storage.get('lis_investigations', MOCK_INVESTIGATIONS);
                    setInvestigations(freshLis);
                  }}
                />
                <Dialog open={isTestDialogOpen} onOpenChange={(open) => {
                  setIsTestDialogOpen(open);
                  if (!open) setEditingTestCode(null);
                }}>
                  ...
                  <DialogContent className="max-w-[550px] rounded-2xl">
                    <DialogHeader>
                      <DialogTitle>{editingTestCode ? 'Edit Investigation Master Test' : 'Add Investigation Master Test'}</DialogTitle>
                    ...
                    <div className="grid grid-cols-2 gap-4 py-4 te">
                      <div className="space-y-1">
                        ...
                        <Input placeholder="e.g. HEM01" value={newTest.code} onChange={e => setNewTest({...newTest, code: e.target.value})} disabled={!!editingTestCode} />
                      </div>
                      <div className="space-y-1">
                    ...
                    <DialogFooter>
                      ...
                      <Button className="bg-indigo-600 text-white hover:bg-indigo-700 text-xs font-semibold" onClick={handleAddTest}>{editingTestCode ? 'Update Test' : 'Save to Database'}</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
            ...
            <CardContent className="p-0">
              <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow>
                    ...
                    <TableHead className="text-center text-[11px] font-bold w-20">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {paginatedInvestigations.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={8} className="py-8 text-center text-slate-500 font-medium">
                        No investigations found matching "{testSearch}".
                      </TableCell>
                    </TableRow>
                  ) : (
                    paginatedInvestigations.map((test) => (
                      <TableRow key={test.code} className="hover:bg-slate-50/50 text-xs border-slate-100">
                        <TableCell className="py-2.5">
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                            <span className="text-[10px] text-muted-foreground uppercase font-black">{test.code} {test.shortName && `(${test.shortName})`}</span>
                            {test.container && (
                              <Badge variant="outline" className="text-[9px] py-0 px-1 font-medium bg-amber-50/50 text-amber-800 border-amber-200">
                                {test.container}
                              </Badge>
                            )}
                            {test.referenceRange && (
                              <span className="text-[10px] text-slate-500 font-mono bg-slate-100 px-1 rounded" title="Reference Range">
              ...
              {filteredInvestigations.length > 0 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-4 border-t border-slate-100 text-xs text-slate-600 bg-slate-50/40">
                  <div className="flex items-center gap-2">
                    <span>Showing</span>
                    <select
                      value={String(investigationPageSize)}
                      onChange={(e) => {
                        setInvestigationPageSize(Number(e.target.value));
                        setInvestigationPage(1);
                      }}
                      className="h-8 rounded-md border border-slate-200 bg-white px-2 py-1 text-xs"
                    >
                      <option value="25">25</option>
                      <option value="50">50</option>
                      <option value="100">100</option>
                      <option value="-1">All ({filteredInvestigations.length})</option>
                    </select>
                    <span>tests per page (Total {filteredInvestigations.length} tests)</span>
                  </div>

                  {investigationPageSize !== -1 && totalInvestigationPages > 1 && (
                    <div className="flex items-center gap-2">
                      <Button 
                        variant="outline" 
                        size="sm" 
                        onClick={() => setInvestigationPage(p => Math.max(1, p - 1))}
                      ...
                      <span className="font-semibold px-2">
                        Page {investigationPage} of {totalInvestigationPages}
                      </span>
                      <Button 
                        variant="outline" 
                        size="sm" 
                        onClick={() => setInvestigationPage(p => Math.min(totalInvestigationPages, p + 1))}
                        disabled={investigationPage >= totalInvestigationPages}
                        className="h-8 px-3 text-xs"
                      >
                        Next
                      </Button>
