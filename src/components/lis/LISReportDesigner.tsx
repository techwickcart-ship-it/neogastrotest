...
export default function LISReportDesigner() {
  ...
  // Dynamic Completed Orders Loading from Supabase and Local Storage
  const fetchCompletedOrders = async () => {
    try {
      const realOrders = await supabaseService.getLabTestRequests();
      let mappedRealRecords: LISResultRecord[] = [];

      if (realOrders && realOrders.length > 0) {
        // Map completed or released DB test requests to LISResultRecords
        const completedOrders = realOrders.filter((o: any) => 
          o.status === 'Completed' || o.status === 'Released' || o.status === 'Verified' || o.result_value || o.findings
        );
        
        mappedRealRecords = completedOrders.map((o: any) => {
          // Determine test code
          let testCode = 'GEN01';
          ...
          if (testCode === 'HEM01') {
            resultsObj['P-HB'] = { parameterId: 'P-HB', parameterName: 'Hemoglobin', value: o.result_value || '14.2', unit: 'g/dL', referenceRangeStr: '12.0 - 17.0 g/dL', status: 'Normal', interpretation: 'Normal' };
            resultsObj['P-RBC'] = { parameterId: 'P-RBC', parameterName: 'Total RBC Count', value: '4.8', unit: 'million/cumm', referenceRangeStr: '4.00 - 5.90 million/cumm', status: 'Normal', interpretation: 'Normal' };
            resultsObj['P-WBC'] = { parameterId: 'P-WBC', parameterName: 'Total Leukocyte Count (TLC)', value: '7500', unit: 'cells/cumm', referenceRangeStr: '4000 - 11000 cells/cumm', status: 'Normal', interpretation: 'Normal' };
            resultsObj['P-PLT'] = { parameterId: 'P-PLT', parameterName: 'Platelet Count', value: '2.5', unit: 'lakh/cumm', referenceRangeStr: '1.50 - 4.50 lakh/cumm', status: 'Normal', interpretation: 'Normal' };
          } else if (testCode === 'BIO01') {
            resultsObj['P-SGOT'] = { parameterId: 'P-SGOT', parameterName: 'SGOT (AST)', value: o.result_value || '28', unit: 'IU/L', referenceRangeStr: '5 - 40 IU/L', status: 'Normal', interpretation: 'Normal' };
            resultsObj['P-SGPT'] = { parameterId: 'P-SGPT', parameterName: 'SGPT (ALT)', value: '31', unit: 'IU/L', referenceRangeStr: '5 - 40 IU/L', status: 'Normal', interpretation: 'Normal' };
          } else if (testCode === 'BIO02') {
            resultsObj['P-UREA'] = { parameterId: 'P-UREA', parameterName: 'Blood Urea', value: o.result_value || '24', unit: 'mg/dL', referenceRangeStr: '15 - 45 mg/dL', status: 'Normal', interpretation: 'Normal' };
            resultsObj['P-CREAT'] = { parameterId: 'P-CREAT', parameterName: 'Serum Creatinine', value: '0.9', unit: 'mg/dL', referenceRangeStr: '0.6 - 1.3 mg/dL', status: 'Normal', interpretation: 'Normal' };
          } else {
            resultsObj['GEN-RES'] = { parameterId: 'GEN-RES', parameterName: o.test_name || 'Observation', value: o.result_value || 'Normal', unit: o.unit || '', referenceRangeStr: o.reference_range || 'Normal biological reference', status: 'Normal', interpretation: '' };
          }

          const patientAgeNum = o.patients?.age || 38;
          const patientGenderObj = o.patients?.gender || 'Male';
          const patientGenderFormatted = (patientGenderObj.charAt(0).toUpperCase() + patientGenderObj.slice(1).toLowerCase()) as 'Male' | 'Female' | 'Other';
          ...
          return {
            ...
            deltaCheckStatus: 'Good' as const,
            deltaCheckMessage: '',
            qrVerified: true,
            verifiedBy: 'Dr. Pradeep Mishra (MD, Pathology)',
            verifiedAt: o.updated_at || o.completed_at || new Date().toISOString(),
            pathologistOpinion: o.findings || 'All investigation values within biological reference range.',
            results: resultsObj,
      ...
      // Combine real records with master mock catalog
      const combined: LISResultRecord[] = [...mappedRealRecords];
      MOCK_LIS_RESULTS.forEach(m => {
        if (!combined.some(c => c.id === m.id || c.patientMRN === m.patientMRN)) {
          combined.push(m);
        }
      });
      ...
      setRecords(MOCK_LIS_RESULTS);
      if (MOCK_LIS_RESULTS.length > 0) {
        setSelectedReport(MOCK_LIS_RESULTS[0]);
      }
    }
  };
  ...
  const filteredRecords = records.filter(r => {
    ...
    const phone = ((r as any).phone || '').toLowerCase();
