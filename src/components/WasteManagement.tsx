...
export default function WasteManagement() {
  ...
  const loadWasteData = useCallback(async () => {
    try {
      const [dbCollections, dbDisposals] = await Promise.all([
        supabaseService.getBioWasteCollections(),
        supabaseService.getBioWasteTransfers()
      ]);
  ...
  const handleDeleteCollection = async (item: any) => {
    if (!confirm('Are you sure you want to delete this waste collection log?')) return;
    try {
      await supabaseService.deleteBioWasteCollection(item.dbId || item.id);
      const updated = collections.filter(c => c.id !== item.id && c.dbId !== item.dbId);
  ...
  const handleDeleteDisposal = async (item: any) => {
    if (!confirm('Are you sure you want to delete this disposal record?')) return;
    try {
      await supabaseService.deleteBioWasteTransfer(item.dbId || item.id);
      const updated = disposals.filter(d => d.id !== item.id && d.dbId !== item.dbId);
      saveDisposals(updated);
      toast.success('Disposal transfer log removed');
    } catch (err) {
      toast.error('Failed to delete disposal log');
    }
  };
  ...
  const handleExportWasteManagement = () => {
    try {
      let csvContent = '';
      let filename = '';

      if (activeTab === 'disposal') {
        const headers = ['Manifest ID', 'Disposal Agency', 'Vehicle No', 'Yellow (KG)', 'Red (KG)', 'White (KG)', 'Blue (KG)', 'Total Weight (KG)', 'Handover Date', 'Officer Name'];
        ...
        filename = `Biomedical_Waste_Disposal_Manifests_${new Date().toISOString().split('T')[0]}.csv`;
      } else {
        const headers = ['Entry ID', 'Color Category', 'Weight (KG)', 'Ward / Department', 'Logged By', 'Collection Date', 'Collection Time', 'Notes'];
        ...
        csvContent = [headers, ...rows].map(row => row.map(cell => `"${String(cell || '').replace(/"/g, '""')}"`).join(',')).join('\n');
        filename = `Biomedical_Waste_Collection_Logs_${new Date().toISOString().split('T')[0]}.csv`;
      }
      ...
      toast.success('Biomedical waste records exported to CSV!');
    } catch {
      toast.error('Failed to export waste records');
    }
  };
    ...
    <div id="biomedical-waste-management" className="p-4 lg:p-8 space-y-6">
      ...
      <div className="bg-[#1A5E63] rounded-3xl p-6 text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-md">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button onClick={handleExportWasteManagement} variant="outline" className="bg-white/10 hover:bg-white/20 text-white border-white/30 font-bold gap-2 rounded-full px-4 py-5 h-auto text-xs shadow-sm cursor-pointer">
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-3 text-right">
                        <Button 
                          ...
                          size="icon" 
                          className="h-7 w-7 text-rose-500 hover:text-rose-700 hover:bg-rose-50"
                          onClick={() => handleDeleteDisposal(item)}
                          title="Delete disposal log"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </Button>
                      </td>
